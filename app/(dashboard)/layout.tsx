    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
      ...
      <aside className="hidden md:flex md:w-60 xl:w-64 md:fixed md:inset-y-0 md:flex-col glass-panel border-r border-gray-100/90 dark:border-gray-800/80 z-30 transition-all duration-300">
        ...
        <nav className="flex-1 p-3.5 space-y-1.5 overflow-y-auto">
          <p className="px-3.5 pb-2 text-[10.5px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
            {lang === 'bn' ? 'মেনু' : 'Menu'}
          </p>
          {sidebarLinks.map((link) => {
            const isActive = pathname === link.href
        ...
        <div className="p-4 border-t border-gray-100/80 dark:border-gray-800/80">
          <div className="glass-card rounded-2xl p-3.5 space-y-3 shadow-xs">
            {isLoading ? (
              <Skeleton className="h-10 w-full rounded-xl" />
            ) : (
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 bg-gradient-to-br from-sky-400 via-cyan-400 to-emerald-400 rounded-xl flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-xs ring-1 ring-white/70 dark:ring-gray-800">
                  {profile?.name?.[0]?.toUpperCase() || 'U'}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-500 dark:text-gray-400 font-normal truncate flex items-center gap-1.5">
            ...
            <div className="flex flex-col gap-2 pt-2 border-t border-gray-100/80 dark:border-gray-800/80">
              ...
              <button
                onClick={handleSignOut}
                className="flex items-center justify-center gap-1.5 w-full py-1.5 rounded-xl text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50/80 dark:hover:bg-red-950/30 transition-all cursor-pointer"
              >
                <LogOut className="h-3.5 w-3.5" />
                <span>{lang === 'bn' ? 'সাইন আউট' : 'Sign Out'}</span>
              </button>
