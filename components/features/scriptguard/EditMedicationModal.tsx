...
interface EditMedicationModalProps {
  ...
  drug: ExtractedMedication | null
  index: number
  allDrugs: ExtractedMedication[]
  onSave: (
    index: number,
...
export function EditMedicationModal({
  ...
  const initialAvatar = inferPillAvatar(
    drug?.brand_name || drug?.written_text || '',
    drug?.dosage || '1 unit'
  )

  const [brandName, setBrandName] = useState(drug?.brand_name || drug?.written_text || '')
  const [genericName, setGenericName] = useState(drug?.generic_name || '')
  const [dosage, setDosage] = useState(drug?.dosage || '1 unit')
  const [frequency, setFrequency] = useState('1+0+1')
  const [mealTiming, setMealTiming] = useState<MealTimingType>('after_meal')
  const [durationDays, setDurationDays] = useState(7)

  const [pillShape, setPillShape] = useState<PillShapeType>(initialAvatar.shape)
  const [pillColor, setPillColor] = useState(initialAvatar.color)
  const [pillColorSecondary, setPillColorSecondary] = useState<string | undefined>(
    initialAvatar.colorSecondary
  )
  const [descriptorBn, setDescriptorBn] = useState(initialAvatar.descriptorBn)

  const [isVerifying, setIsVerifying] = useState(false)
  const [isVerified, setIsVerified] = useState(drug?.mapping_confidence === 'high')
  const [verifiedClass, setVerifiedClass] = useState(drug?.drug_class || '')
  const [recalculatedInteractions, setRecalculatedInteractions] = useState<DrugInteraction[]>([])

  // Live drug verification function
  const handleVerifyDrugName = async (nameToVerify: string) => {
    if (!nameToVerify.trim()) return
    setIsVerifying(true)

    try {
      const otherGenerics = allDrugs
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const updated: ExtractedMedication = {
      ...
      drug_class: verifiedClass || drug.drug_class,
      ...
      <DialogContent className="max-w-md p-6 rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl space-y-4">
        <DialogHeader className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-base font-bold text-gray-900 dark:text-gray-100">
              {isBn ? 'ওষুধের তথ্য সংশোধন ও যাচাই' : 'Edit & Verify Medication'}
            </DialogTitle>
          </div>
          <DialogDescription className="text-xs text-gray-500 dark:text-gray-400">
            {isBn
              ? 'প্রেসক্রিপশন পড়ার ভুল সংশোধন করুন — নাম পরিবর্তনের সাথে সাথে ডাটাবেস দিয়ে যাচাই হবে।'
              : 'Correct misread names or adjust dosing schedule with live database validation.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div className="p-3.5 rounded-2xl bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700/60 flex items-center justify-between gap-3">
            ...
            {isVerified ? (
              ...
              <Badge variant="outline" className="text-[10px] text-amber-600 border-amber-300">
                {isBn ? 'সম্পাদনা চলছে' : 'Editing'}
              </Badge>
            )}
          </div>

          {/* Medicine Name with Verification Button */}
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold text-gray-700 dark:text-gray-300">
              {isBn ? 'ওষুধের নাম (Brand / Generic Name)' : 'Medicine Name'}
            </Label>
            <div className="flex items-center gap-2">
              <Input
                value={brandName}
              ...
              <Button
                ...
                className="h-9.5 text-xs font-bold rounded-xl px-3 shrink-0 border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50"
              >
                {isVerifying ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-gray-700 dark:text-gray-300">
                {isBn ? 'মাত্রা (Dosage / Strength)' : 'Dosage / Strength'}
              </Label>
              <Input
                value={dosage}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-1">
                ...
                <span>{isBn ? 'গ্রহণের নিয়ম (Frequency)' : 'Frequency'}</span>
              </Label>
              <select
                value={frequency}
          ...
          {/* Recalculated Interactions Alert if any */}
          {recalculatedInteractions.length > 0 && (
            <div className="p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 text-amber-800 dark:text-amber-200 text-xs space-y-1">
              <p className="font-bold flex items-center gap-1 text-[11px]">
                ...
                <span>
                  {isBn
                    ? 'নতুন ওষুধে সম্ভাব্য ড্রাগ ইন্টারঅ্যাকশন পাওয়া গেছে:'
          ...
          <div className="pt-2">
            <Button
              type="submit"
              className="w-full rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-bold h-11 text-xs shadow-md"
            >
              <Check className="h-4 w-4 mr-1" />
              <span>{isBn ? 'সংশোধন সম্পন্ন করুন' : 'Save Verified Changes'}</span>
            </Button>
          </div>
        </form>
      </DialogContent>
